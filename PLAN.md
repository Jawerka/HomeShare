...
- `crypto/` — Ed25519 identity, auth token storage helpers, HKDF (если payload encryption), random PIN.
...
- `auth/` — `X-HomeShare-Peer-Id`, `X-HomeShare-Auth-Token`, timestamp + Ed25519 signature.
...
| DB очереди | Drift или JSON (старт с JSON/file store, Drift если разрастётся) |
| Crypto | package crypto + Ed25519 |
| Win installer | Inno Setup 6 |
| Win shell | C++ ATL COM |
| Linux | dart compile exe + systemd + avahi |
| CI | GitHub Actions на тег `v*` |
